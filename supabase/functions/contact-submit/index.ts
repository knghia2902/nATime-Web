...
const ipSalt = requireEnv('NATIME_CONTACT_IP_SALT');
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const mail = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { authorization: `Bearer ${resendApiKey}`, 'content-type': 'application/json' }, body: JSON.stringify({ from: fromEmail, to: [supportEmail], reply_to: email, subject: `[nATime] ${kind}: ${name}`, text: `Request: ${saved.id}\nName: ${name}\nEmail: ${email}\nCompany: ${company ?? '-'}\nType: ${kind}\n\n${message}` }) });
    return json({ requestId: saved.id, emailNotified: mail.ok }, mail.ok ? 201 : 202, cors);
  } catch (error) {
