...
export default function PortalOverview() {
  ...
  const cards = [
    ...
    },
    {
      ...
      iconBg: 'bg-white/[0.06] text-amber-400',
  ...
  return (
    <PortalShell title="Tổng quan" description="Trạng thái license, thiết bị và đơn hàng của tài khoản.">
      <div className="space-y-6 stagger-fade-in">
        {loadIssue && (
          <div className="rounded-xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-400 flex items-start gap-3">
        ...
        {search.get('payment') && (
          <div className="rounded-xl border border-white/20 bg-white/[0.04] p-4 text-sm text-white flex items-start gap-3">
        ...
        {/* Hero banner section */}
        <section className="bg-[rgba(15,23,38,0.85)] border border-white/[0.08] text-white rounded-2xl p-6 sm:p-8 shadow-xl relative overflow-hidden backdrop-blur-xl">
          <div className="relative z-10 grid gap-6 lg:grid-cols-[1fr_auto] lg:items-center">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-white/60 font-medium">
                {summary.licenses > 0
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-[#0a1628] shadow-md hover:bg-white/85 transition-all duration-200 group w-fit"
            >
        ...
        {plan && (
          <section className="rounded-2xl border border-white/20 bg-[rgba(15,23,38,0.85)] p-6 sm:p-7 shadow-sm backdrop-blur">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white">Tiếp tục mua gói {plan === 'standard' ? 'Standard' : 'Professional'}</h2>
                <p className="text-xs text-white/60 font-medium">Thanh toán an toàn qua cổng PayOS</p>
              </div>
            </div>
            <p className="mt-3 text-sm text-white/70 leading-relaxed font-medium">
              Chu kỳ: <span className="font-bold text-white">{billing === 'monthly' ? 'hàng tháng' : 'hàng năm'}</span>. Bạn sẽ được chuyển tới PayOS để thanh toán bằng VietQR/chuyển khoản.
            </p>
            <button
              onClick={checkout}
              disabled={busy !== null}
              className="mt-5 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-2.5 text-sm font-semibold text-[#0a1628] hover:bg-white/85 transition-all shadow-[0_2px_12px_rgba(255,255,255,0.15)] disabled:opacity-60 cursor-pointer"
            >
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <section className="rounded-2xl border border-white/[0.08] bg-[rgba(15,23,38,0.75)] p-6 sm:p-7 shadow-sm flex flex-col justify-between backdrop-blur">
            <div>
              <div className="flex items-center gap-3">
                <div className="grid h-9 w-9 place-items-center rounded-lg bg-white/[0.06] text-white">
                  <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 016 3.75h2.25A2.25 2.25 0 0110.5 6v2.25a2.25 2.25 0 01-2.25 2.25H6a2.25 2.25 0 01-2.25-2.25V6zM3.75 15.75A2.25 2.25 0 016 13.5h2.25a2.25 2.25 0 012.25 2.25V18a2.25 2.25 0 01-2.25 2.25H6A2.25 2.25 0 013.75 18v-2.25zM13.5 6a2.25 2.25 0 012.25-2.25H18A2.25 2.25 0 0120.25 6v2.25A2.25 2.25 0 0118 10.5h-2.25a2.25 2.25 0 01-2.25-2.25V6zM13.5 15.75a2.25 2.25 0 012.25-2.25H18a2.25 2.25 0 012.25 2.25V18A2.25 2.25 0 0118 20.25h-2.25A2.25 2.25 0 0113.5 18v-2.25z" />
                  </svg>
                </div>
                <h2 className="text-lg font-bold text-white">Hành động nhanh</h2>
              </div>
              <div className="mt-4 flex flex-wrap gap-3">
                <Link
                  href="/portal/licenses"
                  className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-semibold text-white hover:bg-white/10 hover:border-white/20 transition-all duration-200 shadow-sm"
                >
        ...
        {message && (
          <div className="rounded-xl border border-white/[0.08] bg-white/[0.04] p-4 text-sm text-white flex items-start gap-3">
