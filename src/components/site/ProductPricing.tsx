    ...
    <div>
      <div className="flex justify-center">
        <div className="inline-flex rounded-md border border-slate-300 bg-white p-1" role="group" aria-label={vi ? 'Chu kỳ thanh toán' : 'Billing period'}>
          <button type="button" onClick={() => setBilling('monthly')} className={`rounded px-4 py-2 text-sm font-semibold ${billing === 'monthly' ? 'bg-slate-900 text-white' : 'text-slate-600'}`}>{vi ? 'Hàng tháng' : 'Monthly'}</button>
          <button type="button" onClick={() => setBilling('yearly')} className={`rounded px-4 py-2 text-sm font-semibold ${billing === 'yearly' ? 'bg-slate-900 text-white' : 'text-slate-600'}`}>{vi ? 'Hàng năm' : 'Yearly'}</button>
        </div>
      </div>
      <div className={`mt-8 grid gap-5 ${compact ? 'lg:grid-cols-3' : 'lg:grid-cols-3'}`}>
        {plans.map((code) => {
          const product = products.find((item) => item.plan_code === code && item.billing_period === billing) ?? verifiedCatalog.find((item) => item.plan_code === code && item.billing_period === billing)!;
        ...
        <article className="flex flex-col rounded-xl border border-slate-200 bg-slate-50 p-6">
          ...
          <p className="mt-5 flex-1 text-sm leading-6 text-slate-600">{vi ? 'Dành cho nhu cầu triển khai và giới hạn cần được khảo sát riêng.' : 'For deployments and limits that require a separate assessment.'}</p>
          <Link href={`${vi ? '' : '/en'}/contact?type=enterprise`} className="mt-7 rounded-md border border-slate-300 bg-white px-4 py-3 text-center text-sm font-semibold text-slate-800 hover:bg-slate-100">{vi ? 'Gửi yêu cầu' : 'Send inquiry'}</Link>
        </article>
      </div>
      <p className="mt-5 text-center text-xs text-slate-500">{catalogLive ? (vi ? 'Giá và giới hạn được tải từ danh mục sản phẩm nATime.' : 'Pricing and limits loaded from the nATime product catalog.') : (vi ? 'Đang hiển thị danh mục giá đã xác minh gần nhất.' : 'Showing the latest verified pricing catalog.')}</p>
    </div>
  );
}
