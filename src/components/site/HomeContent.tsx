    ...
    <PublicShell locale={locale}>
      <section className="relative flex min-h-[calc(100vh-4rem)] flex-col justify-center overflow-hidden bg-background">
        <div className="pointer-events-none absolute inset-0" aria-hidden="true">
          <div className="absolute -top-40 left-1/2 h-[700px] w-[900px] -translate-x-1/2 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(59,130,246,.32)_0%,transparent_70%)] opacity-30 blur-3xl" />
          <div className="absolute bottom-0 right-0 h-[400px] w-[500px] translate-x-1/4 translate-y-1/4 rounded-full bg-[radial-gradient(ellipse_at_center,rgba(37,99,235,.36)_0%,transparent_70%)] opacity-20 blur-3xl" />
          <div className="absolute inset-0 opacity-[0.025] [background-image:linear-gradient(rgba(37,99,235,1)_1px,transparent_1px),linear-gradient(90deg,rgba(37,99,235,1)_1px,transparent_1px)] [background-size:72px_72px]" />
        </div>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl px-4 pb-20 pt-24 sm:px-6 md:pb-24 md:pt-28 lg:px-8">
          <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2 lg:gap-16">
            <div className="flex flex-col items-start">
              <h1 className="mb-5">
                <span className="mb-3 block text-5xl font-black leading-[1.05] tracking-tight text-foreground sm:text-6xl lg:text-7xl">nATime</span>
                <span className="block text-2xl font-extrabold leading-snug tracking-tight sm:text-3xl lg:text-[2rem]">
                  <span className="text-foreground">{vi ? 'Giải pháp Chấm công' : 'Time Attendance'}</span>
                  <br />
                  <span className="text-primary">{vi ? '& Quản lý Thiết bị cho Doanh nghiệp' : '& Business Device Management'}</span>
                </span>
              </h1>

              <p className="mb-9 max-w-xl text-base leading-relaxed text-muted sm:text-lg">{vi ? 'Hệ thống cài đặt trên Windows giúp doanh nghiệp quản lý chấm công, máy chấm công và quyền sử dụng phần mềm trong cùng một quy trình.' : 'A Windows-installed system for managing attendance, attendance devices and software access in one operational flow.'}</p>

              <div className="mb-12 flex w-full flex-col items-start gap-3 sm:w-auto sm:flex-row sm:items-center">
                <Link href="/register?trial=standard" className="inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-primary px-7 text-sm font-bold text-primary-foreground shadow-lg shadow-primary/25 transition hover:-translate-y-0.5 hover:bg-primary/90 hover:shadow-xl sm:w-auto">{vi ? 'Dùng thử miễn phí' : 'Start free trial'}<span aria-hidden="true">→</span></Link>
                <Link href={`${prefix}/contact`} className="inline-flex h-12 w-full items-center justify-center rounded-xl border border-border bg-background/85 px-7 text-sm font-semibold text-foreground transition hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50/50 sm:w-auto">{vi ? 'Liên hệ tư vấn' : 'Contact sales'}</Link>
              </div>

              <div className="flex flex-wrap items-center gap-4 text-xs font-medium text-muted">
                <span className="rounded-full border border-border bg-background/60 px-3 py-1.5">✓ Windows x64</span>
                <span className="rounded-full border border-border bg-background/60 px-3 py-1.5">✓ {vi ? 'Trial Standard 7 ngày' : '7-day Standard trial'}</span>
                <span className="rounded-full border border-border bg-background/60 px-3 py-1.5">✓ {vi ? 'License theo máy' : 'Machine-bound license'}</span>
              </div>
            </div>

            <div className="relative flex items-center justify-center lg:justify-end">
              <div className="absolute inset-0 -z-10 rounded-2xl bg-[radial-gradient(ellipse_at_center,rgba(37,99,235,.25)_0%,transparent_70%)] opacity-40 blur-2xl" />
              <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-border/80 bg-card shadow-[0_20px_80px_-10px_rgba(15,23,42,.20)] transition duration-700 hover:-translate-y-1 lg:max-w-none">
                <div className="flex select-none items-center justify-between border-b border-border bg-muted/50 px-4 py-3">
                  ...
                  <div className="flex w-full max-w-[240px] items-center justify-center gap-2 rounded-md border border-border bg-background px-4 py-1 font-mono text-[11px] text-muted"><span className="text-emerald-600" aria-hidden="true">●</span><span className="truncate">nATime · Windows</span></div>
              ...
              <div className="absolute -bottom-5 -left-3 hidden items-center gap-3 rounded-xl border border-border/80 bg-background/90 px-4 py-3 text-xs shadow-xl backdrop-blur-xl sm:flex">
                <span className="grid h-8 w-8 place-items-center rounded-lg bg-emerald-50 text-emerald-600">✓</span>
                <div><p className="text-[10px] text-muted">{vi ? 'Phát hành' : 'Release'}</p><p className="font-bold text-foreground">{vi ? 'Bộ cài Windows đã xác minh' : 'Verified Windows installer'}</p></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-[1200px] px-4 py-20 sm:px-6 lg:py-24">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">{values.map(([number, title, text]) => <article key={number} className="group rounded-2xl border border-border bg-card p-7 transition hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-slate-200/60"><div className="flex items-center justify-between"><span className="text-xs font-black tracking-[0.18em] text-blue-600">{number}</span><span className="h-px w-12 bg-border transition group-hover:bg-blue-300" /></div><h3 className="mt-8 text-xl font-bold text-foreground">{title}</h3><p className="mt-3 text-sm leading-6 text-muted">{text}</p></article>)}</div>
      </section>

      <section className="overflow-hidden border-y border-border bg-slate-950 text-white">
      ...
      <section className="bg-muted/30"><div className="mx-auto max-w-[1200px] px-4 py-20 sm:px-6 lg:py-24"><SectionHeading eyebrow={vi ? 'Bắt đầu' : 'Getting started'} title={vi ? 'Bốn bước từ tài khoản đến vận hành' : 'Four steps from account to operation'} description={vi ? 'Quy trình cài đặt và kích hoạt được thiết kế để doanh nghiệp tự kiểm tra từng bước.' : 'The installation and activation flow lets the business verify every step.'} /><div className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{steps.map(([title, text], index) => <article key={title} className="relative rounded-xl border border-border bg-card p-6"><span className="grid h-9 w-9 place-items-center rounded-lg bg-blue-50 text-sm font-black text-blue-600">{index + 1}</span><h3 className="mt-5 font-bold text-foreground">{title}</h3><p className="mt-2 text-sm leading-6 text-muted">{text}</p></article>)}</div></div></section>
      ...
      <section className="px-4 pb-20 sm:px-6"><div className="mx-auto max-w-[1200px] overflow-hidden rounded-3xl bg-[linear-gradient(120deg,#0f172a,#1e3a5f_58%,#2563eb)] px-6 py-12 text-white shadow-2xl shadow-blue-950/20 sm:px-10 lg:flex lg:items-center lg:justify-between lg:px-14 lg:py-14"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-blue-200">nATime Standard</p><h2 className="mt-3 text-3xl font-black sm:text-4xl">{vi ? 'Kiểm chứng quy trình trên một máy Windows' : 'Validate the flow on one Windows machine'}</h2><p className="mt-4 max-w-2xl leading-7 text-blue-100">{vi ? 'Trial 7 ngày, tối đa 50 nhân sự, một thiết bị và module Standard hiện hành.' : 'A 7-day trial for up to 50 employees, one device and the current Standard modules.'}</p></div><div className="mt-8 flex shrink-0 flex-col gap-3 sm:flex-row lg:mt-0 lg:ml-10"><Link href="/register?trial=standard" className="rounded-lg bg-white px-6 py-3.5 text-center text-sm font-bold text-slate-950 hover:bg-blue-50">{vi ? 'Tạo tài khoản' : 'Create account'}</Link><Link href={`${prefix}/download`} className="rounded-lg border border-white/30 px-6 py-3.5 text-center text-sm font-bold text-white hover:bg-white/10">{vi ? 'Tải bộ cài' : 'Download'}</Link></div></div></section>
    </PublicShell>
  );
}
...
function SectionHeading({ eyebrow, title, description, dark = false, centered = false }: { eyebrow: string; title: string; description: string; dark?: boolean; centered?: boolean }) {
  return <div className={centered ? 'mx-auto max-w-3xl text-center' : 'max-w-3xl'}><p className={`text-xs font-black uppercase tracking-[0.18em] ${dark ? 'text-blue-300' : 'text-blue-600'}`}>{eyebrow}</p><h2 className={`mt-3 text-3xl font-black tracking-[-0.025em] sm:text-4xl ${dark ? 'text-white' : 'text-foreground'}`}>{title}</h2><p className={`mt-4 text-base leading-7 ${dark ? 'text-slate-300' : 'text-muted'}`}>{description}</p></div>;
}

function InfoStrip({ title, value }: { title: string; value: string }) {
  return <div className="px-5 py-6 sm:px-7"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-blue-600">{title}</p><p className="mt-2 text-sm font-semibold text-foreground">{value}</p></div>;
}
